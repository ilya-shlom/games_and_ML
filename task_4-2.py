...
input_file = open("task_4-2.csv", "w")
input_file.write("#;Вероятность А;Вероятность B;Выигрыш;Средний выигрыш;"
                 "Мат.ожидание;СКО;Дисперсия;Теоретическое СКО\n")
iterations = 0


# Проведение эксперимента с машинным обучением - обучение с наказанием
def game(probability_b, board):
    global iterations
    balls_a = [0, 1] * 100
...
# Задание 4-2
print("Задание 4-2: Машинное обучение с наказанием")
for i in range(5):
    print("Игра ", i + 1, ":", sep="")
    game(0.5, main_board)
    print("")
